...
// 2.1 读取某个目录下所有文件的文件名，返回的是一个字符串数组。注意这个在后面批量注册路由会用到。
//同步方法
// let result =fs.readdirSync('./') // 
// console.log(result)
// console.log(typeof result)

//异步方法 fs.readdir('./',{encoding:'buffer',withFileTypes:true},callback)，注意第二个参数是一个对象encoding默认是utf-8可以设置成其它数据输出类型如buffer，
...
//     }
// })
// 自己封装成一个promise，形参path可以通过path模块解析
...
// 2.2 读取文件里的内容，node会缓存整个文件
//同步方法
// const result = fs.readFileSync('../../public/readfiletest.txt',{encoding:"utf-8"})
// console.log(result)//这是我读取到的内容,不指定编码格式时返回的是buffer
...
// 上面的封装到一个普通函数中（callback方式）获取一个文件的内容  对调地狱  -  callback-hell
...
// 2.3 写文件,把新的内容写入文件中，没有返回值或者说返回值是undefined。
//同步方法
...
// 异步方法：fs.writeFile的别名 fs.appendFile 可以说是也可将数据追加到文件，如果文件尚不存在则创建该文件。
// const fullFileName = path.join(__dirname,'../','../','public','readfiletest.txt')
// const data = '/n 我是用appendFile方法新写入的内容它不会覆盖之前文件所有的内容'
// fs.appendFile(fullFileName,data,{encoding:'utf-8'}, (err,data) => {
//     if (err) throw err;
//     console.log('数据已追加到文件',data);
// });

// 2.4 文件流的读取写入，读写日志时使用。
//文件拷贝的原理是通过fs.readFile从一个文件读取内容，然后通过fs.writeFile将其写入另一个文件。
//readFile会默认将文件内容全部读取到内存中，然后再写入另一个文件。但是这种拷贝文件的方式，适用是文件较小时（小于64k）。当大于64k时，会出现性能问题。
...
//   return promise
// }
// mkdir(fullFileName).then(msg => {console.log(msg)}).catch(err => {console.log('创建文件目录失败:',err)})